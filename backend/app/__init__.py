...
def add(param1, param2):
    ...
    time.sleep(2)
    #clientId.emit('progress', {'status': 100, 'result': str(res.result)}, namespace='/cruncher')
    socketio.emit('progress', {'status': 100, 'result': str(res.result)}, namespace='/cruncher')
    return jsonify(task_id=task.id, status_url=url_for('check_task', id=task.id, _external=True))
